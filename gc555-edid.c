...
/*
 * GC573 EDID: derived from the GC555 blob with the GC573 identity
 * applied (product code 0x5730, monitor name "AVT GC573"); the two
 * cards share the same receiver capability set.
 */
static const u8 gc573_edid[GC555_EDID_SIZE] = {
	0x00, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x00, 0x06, 0xd8, 0x30, 0x57,
	0x00, 0x00, 0x00, 0x00, 0x22, 0x1d, 0x01, 0x03, 0x80, 0xa0, 0x5a, 0x78,
	0xea, 0x08, 0xa5, 0xa2, 0x57, 0x4f, 0xa2, 0x28, 0x0f, 0x50, 0x54, 0x25,
	0x0b, 0x00, 0xd1, 0xc0, 0x81, 0x40, 0x81, 0x80, 0x81, 0x00, 0x8b, 0xc0,
	0x95, 0x00, 0xb3, 0x00, 0x81, 0xc0, 0x08, 0xe8, 0x00, 0x30, 0xf2, 0x70,
	...
	0x32, 0x40, 0x30, 0x30, 0xca, 0x00, 0x1e, 0x4e, 0x31, 0x00, 0x00, 0x1e,
	0x00, 0x00, 0x00, 0xfc, 0x00, 0x41, 0x56, 0x54, 0x20, 0x47, 0x43, 0x35,
	0x37, 0x33, 0x0a, 0x20, 0x20, 0x20, 0x01, 0xe9, 0x02, 0x03, 0x37, 0xf1,
	0x4c, 0x61, 0x60, 0x5f, 0x5e, 0x5d, 0x1f, 0x5a, 0x3f, 0x13, 0x22, 0x21,
	0x20, 0x23, 0x0f, 0x07, 0x07, 0x83, 0x4f, 0x00, 0x00, 0x67, 0x03, 0x0c,
	0x00, 0x10, 0x00, 0x38, 0x3c, 0x67, 0xd8, 0x5d, 0xc4, 0x01, 0x78, 0x80,
	0x03, 0xe2, 0x00, 0xcf, 0xe3, 0x05, 0xc0, 0x00, 0xe2, 0x0f, 0x03, 0xe3,
			     ...
			     size_t *size)
{
	...
	/*
	 * The stock GC573 driver publishes a GC573-specific EDID; keep one
	 * table per model so the product and capability data match the card.
	 */
	table = model == GC555_MODEL_GC573 ? gc573_edid : gc555_edid;
	if (!gc555_edid_valid(table, GC555_EDID_SIZE))
		return -EINVAL;

	*edid = table;
	*size = GC555_EDID_SIZE;
	return 0;
}

int gc555_edid_get(const u8 **edid, size_t *size)
{
	return gc555_edid_get_for_model(GC555_MODEL_GC555, edid, size);
}
